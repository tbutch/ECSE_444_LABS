...
void dot_product_c (float * Array_A, float * Array_B, int arrayLength, float * result);
void varianceInC (float * Array, int arrayLength, float * result);
...
int main() {
	...
	/* C Dot Product */
	dot_product_c(f1000_array,f1000_array,1000,&dotpC);
	
	/* CMSIS Dot Product */
	arm_dot_prod_f32 (f1000_array, f1000_array, 1000, &dotpCMSIS);
	
	/* ASM Dot Product */
	asm_dot_product(f1000_array, f1000_array, 1000, &dotpASM);
	
	printf("C Dot Product = %f\n", dotpC);
	printf("CSMIS_DSP Dot Product = %f\n",dotpCMSIS);
	printf("ASM Dot Product = %f\n", dotpASM);
	
	/* C Variance */
	varianceInC(f1000_array, 1000, &varianceC);
	
	/* CMSIS Variance */
	arm_var_f32 (f1000_array, 1000, &varianceCMSIS);
	
	/* ASM Variance */
	asm_variance(f1000_array, 1000, &varianceASM);
	
	printf("C Variance = %f\n",varianceC);
	printf("CMSIS-DSP Variance = %f\n",varianceCMSIS);
	printf("ASM Variance = %f\n", varianceASM);
	return 0;
}
...
void varianceInC (float * Array, int arrayLength, float * result){
	...
	float mean=0;
	float var=0;
	float floatArrayLength = (float) arrayLength;
	for(i=0;i<arrayLength; i++){
		mean+=(*(Array+i));
	}
	mean = mean/floatArrayLength;
	// Calculate variance
	for(i=0; i<arrayLength; i++){
		var+=(((*(Array+i)) - mean) * ((*(Array+i)) - mean));
	}
	*result = (var/(floatArrayLength-1));
}
